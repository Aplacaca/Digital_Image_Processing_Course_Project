...
class SSDLoss(nn.Module):
    ...
    def __init__(self, n_classes: int, variance=(0.1, 0.2), overlap_thresh=0.5, neg_pos_ratio=3, device='cuda:0', **kwargs):
        """
        Parameters
        ----------
        n_classes: int
    ...
    def forward(self, pred: Tuple[Tensor, Tensor, Tensor], target: List[Tensor]):
        ...
        target: list of shape `(N, )`
            标签列表，每个标签的形状为 `(n_objects, 5)`，第二个维度包含[x1, y1, x2, y2, class_id]，每张图中有n_objects个目标
        """
        loc_pred, conf_pred, prior = pred
        N = conf_pred.size(0)  # batch_size
        n_priors = prior.size(0)  # anchor_num

        # 将先验框和边界框 ground truth 匹配
        loc_t = torch.Tensor(N, n_priors, 4)  # 保存编码后的偏移量
        conf_t = torch.Tensor(N, n_priors)  # 保存编码后的类别id，从背景0开始
        prior = prior.detach()
        for i in range(N):
