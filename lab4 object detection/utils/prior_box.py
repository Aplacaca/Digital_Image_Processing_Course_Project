...
if __name__ == "__main__":
    ...
    from visualize import show_bboxes
    from box_utils import center_to_corner
    prior = PriorBox()
    prior = center_to_corner(prior())
    img = torch.tensor(np.load('../img.npy')[0])

    img = transforms.ToPILImage()(img.clone().squeeze(0))
    fig = plt.imshow(img)
    plt.title('anchors')
    bbox_scale = torch.tensor([300]*4)
    for i in range(7006, 7012):
        show_bboxes(fig.axes, [prior[i] * bbox_scale])
    for i in range(1000, 1004):
        show_bboxes(fig.axes, [prior[i] * bbox_scale])
    for i in range(8196, 8200):
        show_bboxes(fig.axes, [prior[i] * bbox_scale])
    plt.show()
