...
def save_result(epoch, opt, real_imgs, fake_imgs,feature_extractor, generator):
...
def save_model(epoch, i, opt, feature_extractor, generator, discriminator):

    # save model
        
    # torch.save(feature_extractor.state_dict(), opt.save_model_file + opt.img_class + '/' + f"fe_{epoch}.pth")
    torch.save(generator.state_dict(), opt.save_model_file + opt.img_class + '/' + f'generator_{epoch}_{i}.pth')
    torch.save(discriminator.state_dict(), opt.save_model_file + opt.img_class + '/' + f'discriminator_{epoch}_{i}.pth')